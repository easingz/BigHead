...
typedef struct xode_pool_struct
{
    ...
    /* added by guojiang li */
    struct xode_pool_free *cleanupTail;
...
char *xode_utf8escape(xode_pool p, char *buf);
...
typedef struct xode_struct
{
     ...
     unsigned short       type;
     char*                data;
     int                  data_sz;  /* ��Xode����ݶ����ó��� */
     int                  data_len; /* ��Xode����ݶο��ó��� */
     int                  complete;
     xode_pool            p;
...
xode xode_get_i_tag(xode parent, const char* name, int i);
...
xode  xode_get_lastattrib(xode parent);
...
xode xode_find_cdata(xode parent, char *tagname, char *cdata);

void *xode_pool_realloc(xode node, xode_pool p, const char *cdata,  int size);
...
#define XODE_REQ_ITEM_TAG(pJabberPDU, elem)     xode_get_tag(pJabberPDU->req, "query/item/"elem)
#define XODE_RESP_ITEM_TAG(pJabberPDU, elem)    xode_get_tag(pJabberPDU->resp, "query/item/"elem)

#define XODE_REQ_ITEM_iTAG(pJabberPDU, elem, index)     xode_get_i_tag(pJabberPDU->req, "query/item/"elem, index)
#define XODE_RESP_ITEM_iTAG(pJabberPDU, elem, index)    xode_get_i_tag(pJabberPDU->resp, "query/item/"elem, index)


#define XODE_REQ_ITEM_CDATA(pJabberPDU, elem)   xode_get_tagdata(pJabberPDU->req, "query/item/"elem)
#define XODE_RESP_ITEM_CDATA(pJabberPDU, elem)  xode_get_tagdata(pJabberPDU->resp, "query/item/"elem)

#define XODE_TAG(pJabberPDU, elem)      xode_get_tag(pJabberPDU->req, "query/item/"elem)
#define XODE_CDATA(pJabberPDU, elem)    xode_get_tagdata(pJabberPDU->req, "query/item/"elem)
